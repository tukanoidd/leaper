...
use db::{
    DB, DBAction, DBNotification, InstrumentedDBQuery,
    apps::{CreateAppEntryQuery, LiveSearchAppsQuery},
    fs::FSNode,
    init_db,
};

use leaper_daemon::{ADDRESS, LeaperDaemon};
...
static DB_REF: OnceLock<DB> = OnceLock::new();
...
impl LeaperDaemon for LeaperDaemonServer {
    ...
    async fn index(self, _context: ::tarpc::context::Context, root: PathBuf, parents: bool) {
        ...
        index(root, parents, |_| None).await
    }
}
